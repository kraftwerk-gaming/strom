...
/* ---------------------------------------------------------------------
   Additions for strom's GameNative build (see default.nix). The imagefs
   GameNative unpacks is Winlator's, built with /data/data/com.winlator
   baked into libxcb's X socket path, fontconfig, the X11 locale tables
   and hundreds of RUNPATHs; these wrappers cover the calls wine, box64
   and the imagefs libraries reach those paths through that the set
   above does not. */
...
WRAPPER(int, __lxstat)(int ver, const char * path, struct stat * st)
{
    int (*__lxstat_real) (int, const char *, struct stat *) = LOOKUP_REAL(__lxstat);
    char buf[PATH_MAX];
    return __lxstat_real(ver, rewrite(path, buf), st);
}
WRAPPER_DEF(__lxstat)

WRAPPER(int, __lxstat64)(int ver, const char * path, struct stat64 * st)
{
    int (*__lxstat64_real) (int, const char *, struct stat64 *) = LOOKUP_REAL(__lxstat64);
    char buf[PATH_MAX];
    return __lxstat64_real(ver, rewrite(path, buf), st);
}
WRAPPER_DEF(__lxstat64)

WRAPPER(ssize_t, readlink)(const char * path, char * out, size_t len)
{
    ssize_t (*readlink_real) (const char *, char *, size_t) = LOOKUP_REAL(readlink);
    char buf[PATH_MAX];
    return readlink_real(rewrite(path, buf), out, len);
}
WRAPPER_DEF(readlink)

WRAPPER(ssize_t, readlinkat)(int dirfd, const char * path, char * out, size_t len)
{
    ssize_t (*readlinkat_real) (int, const char *, char *, size_t) = LOOKUP_REAL(readlinkat);
    char buf[PATH_MAX];
    return readlinkat_real(dirfd, rewrite(path, buf), out, len);
}
WRAPPER_DEF(readlinkat)

WRAPPER(char *, realpath)(const char * path, char * resolved)
{
    char * (*realpath_real) (const char *, char *) = LOOKUP_REAL(realpath);
    char buf[PATH_MAX];
    return realpath_real(rewrite(path, buf), resolved);
}
WRAPPER_DEF(realpath)

WRAPPER(int, rename)(const char * from_path, const char * to_path)
{
    int (*rename_real) (const char *, const char *) = LOOKUP_REAL(rename);
    char buf1[PATH_MAX], buf2[PATH_MAX];
    return rename_real(rewrite(from_path, buf1), rewrite(to_path, buf2));
}
WRAPPER_DEF(rename)

WRAPPER(int, symlink)(const char * target, const char * linkpath)
{
    int (*symlink_real) (const char *, const char *) = LOOKUP_REAL(symlink);
    char buf1[PATH_MAX], buf2[PATH_MAX];
    return symlink_real(rewrite(target, buf1), rewrite(linkpath, buf2));
}
WRAPPER_DEF(symlink)

/* Fortified open() entry points (-D_FORTIFY_SOURCE builds call these
   instead of open/open64 when the flags are a compile-time constant). */
WRAPPER(int, __open_2)(const char * path, int flags)
{
    int (*__open_2_real) (const char *, int) = LOOKUP_REAL(__open_2);
    char buf[PATH_MAX];
    return __open_2_real(rewrite(path, buf), flags);
}
WRAPPER_DEF(__open_2)

WRAPPER(int, __open64_2)(const char * path, int flags)
{
    int (*__open64_2_real) (const char *, int) = LOOKUP_REAL(__open64_2);
    char buf[PATH_MAX];
    return __open64_2_real(rewrite(path, buf), flags);
}
WRAPPER_DEF(__open64_2)

WRAPPER(int, __openat_2)(int dirfd, const char * path, int flags)
{
    int (*__openat_2_real) (int, const char *, int) = LOOKUP_REAL(__openat_2);
    char buf[PATH_MAX];
    return __openat_2_real(dirfd, rewrite(path, buf), flags);
}
WRAPPER_DEF(__openat_2)

WRAPPER(int, __openat64_2)(int dirfd, const char * path, int flags)
{
    int (*__openat64_2_real) (int, const char *, int) = LOOKUP_REAL(__openat64_2);
    char buf[PATH_MAX];
    return __openat64_2_real(dirfd, rewrite(path, buf), flags);
}
WRAPPER_DEF(__openat64_2)

/* The dynamic loader opens libraries with its own syscalls, so a
   RUNPATH or an absolute dlopen() path never passes through open()
   above; the string has to be rewritten before it reaches ld.so. Only
   absolute paths are rewritten -- a bare soname must keep going through
   the search path. */
WRAPPER(void *, dlopen)(const char * path, int flags)
{
    void * (*dlopen_real) (const char *, int) = LOOKUP_REAL(dlopen);
    char buf[PATH_MAX];
    if (path != NULL && path[0] == '/') path = rewrite(path, buf);
    return dlopen_real(path, flags);
}
WRAPPER_DEF(dlopen)
