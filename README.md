...
The catalog (`web/catalog.json`) is Steam-enriched metadata generated by
`scripts/fetch-steam-metadata.py`; games without a Steam match fall back to
their lutris banner so every game still gets a tile. New games appear
automatically once the metadata script is re-run — no GUI code changes.
...
proper genres, screenshots, player-mode tags, etc., drop a `gui.json` next to
the game's `default.nix`:

```jsonc
// games/battlefield-2/gui.json
{
  "genres": ["Action", "FPS"],
...
`developers`, `hero`, `screenshots`, `lutris`. The exact `tags` strings that map
to each Features filter are documented in
[AGENTS.md](AGENTS.md#web-gui-catalog-metadata). To control Steam matching
itself, edit `scripts/steam-overrides.json` (slug → appid, or `null` to skip).
...
4. Run `python3 scripts/fetch-steam-metadata.py` to refresh the web catalog
   (`web/catalog.json`) so the game gets a GUI tile. If it is not on Steam, add
   a `games/<slug>/gui.json` with manual metadata (see
   [Web GUI metadata](#manual-metadata-for-non-steam-games)); for a wrong Steam
   match, pin or skip the appid in `scripts/steam-overrides.json`. Commit the
   regenerated `web/catalog.json`.
