...
- **Pin the game tree, not the installer.** The shape every game converges on is `src = fetchIpfs { cid = "bafy..."; directory = true; hash = "<nix hash path of the tree>"; name = "<slug>"; size = <bytes>; }` with no `buildScript`: the desktop overlays that tree and the Android client unpacks it, from the same CID, and nothing is extracted or patched at build or run time on either side. A recipe that still starts from an archive and unpacks it in `buildScript` is the pre-pin state: build its tree once (`nix build .#androidPayloads.<slug>`), pin it, then replace `src` with the tree and delete the extraction. Mods follow the same rule: every `modLayers` entry's `tree` is a fetched tree (`nix build '.#androidLayerPayloads.<slug>."<layer>"'` produces it), and a toggle's files -- FF8's FFNx included -- live in a layer, never in the base, because a phone cannot rebuild a base. See `games/animal-well` (one tree) and `games/final-fantasy-viii` (a base plus 15 layers).
- A fetched tree is walked one directory block at a time through the gateways (`lib/ipfs-walk.py`) and its files are downloaded by path. Trees must not contain symlinks; exec bits are preserved only if the tree was added with `--preserve-mode`, so `hash` is the NAR hash of exactly what a gateway serves -- take it from the store build only when that has no exec bits (`find -perm -u+x`), else from the fetch error.
- CIDs in this repo are generated with `ipfs add --raw-leaves` (files) or `ipfs add -r --cid-version=1` (trees; the pin host's upload endpoint uses exactly that). A plain `ipfs add` without `--raw-leaves` produces a **different CID** for the same file. Always use `--raw-leaves` when adding files to match the CIDs in this repo. (`--nocopy` implies `--raw-leaves`; `--only-hash` does NOT, so it must be paired with `--raw-leaves` explicitly.)
...
- Public gateways differ in what they find cold: measured 2026-09-06 against a freshly pinned CID, `gateway.pinata.cloud` and `nftstorage.link` resolve it; `ipfs.io`, `dweb.link`, `w3s.link` and `trustless-gateway.link` answer 504 until something has warmed their cache, then serve fastest. Both fetchers list them in that order. All of them rate-limit by request count: a tree is fetched a few files at a time, one connection per gateway.
