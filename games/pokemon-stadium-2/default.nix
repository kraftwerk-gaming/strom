...
let
  # Pokemon Stadium 2 (NTSC-U, 2001 Nintendo). 32 MiB N64 ROM in native
  # big-endian z64 format (magic 0x80371240). mupen64plus-next runs it
  # without extra config or a Transfer Pak.
  rom = fetchIpfs {
    cid = "QmXGpzBBmC7hrqBMAbpToLND81ruQuc3pGb9ixxnGdmYag";
    fallbackUrl = "https://archive.org/download/20240625_20240625_2228/Pokemon%20Stadium%202%20%28USA%29.z64";
    hash = "sha256-gWCr59S5c0MXAAxE6fiC7+/kzGNZ6s3tmL5i8oEeQKk=";
    name = "pokemon-stadium-2.z64";
  };
...
self.lib.mkGame { inherit lib pkgs; } {
  ...
  src = rom;
  runtime = "retroarch";
  executable = "pokemon-stadium-2.z64";
  ...
  retroarch = {
    ...
    # Keyboard layout matching the other N64 games in this flake (see
    # super-mario-64 for the libretro->N64 mapping rationale).
    settings = {
      ...
      # Left analog stick (movement / menu navigation).
  ...
  meta = {
    description = "Pokemon Stadium 2 (2001 Nintendo, N64, via RetroArch / mupen64plus-next)";
    mainProgram = "pokemon-stadium-2";
