  ...
  imports = [
    (
      ...
      let
        ...
        # Tree containing ECM (ASI + INI + BASS) under `SCRIPTS/` plus
        # ONLY `SCRIPTS/Music/<sanitized-basename>` symlinks into the
        # user's extraSoundtracks derivations. Shipped as a separate
        # overlay lower above the pinned game tree, so toggling the music
        # player or changing the soundtrack list never touches the base
        # (one fixed pinned artifact). The kernel-overlay merge at
        # /SCRIPTS/ exposes both the base tree's SCRIPTS contents
        # (DeadlockFix.asi, ...) and this layer's files transparently.
        # Runtime user-drops in
        ...
        soundtracks = runCommandLocal "need-for-speed-underground-2-soundtracks" { } ''
          ...
          # ECM (external custom music) ASI + bass.dll + INI. The release
          # zip ships them under `scripts/` but fetchzip strips that root;
          # we drop the files into `SCRIPTS/` alongside the other ASI mods
          # loaded by dinput8.dll. ECM reads the `playlist` dir from its
          # INI relative to the ASI; default `playlist = "Music"` ->
          # `SCRIPTS/Music/`.
      ...
      in
      {
        options = {
          ...
          extraSoundtracks = mkOption {
            ...
            description = ''
              ...
              the pinned game tree via `bwrap.overlay.lowers`, so
              adding/removing tracks never re-fetches the game, and the
              files do NOT live in the base tree or in the user's
              `~/.strom/...` state. Runtime user-drops in
        ...
        config = {
          ...
          # https://archive.org/download/NFSU2Stable/Need%20for%20Speed%20Underground%202.7z),
          # extracted with its "Need for Speed Underground 2" root folded
          # away, plus two modifications baked into the tree:
          #
          #  - SPEED2.EXE: the serial-port Sleep(0) spinloops are NOP'd
          #    (two bytes \x90\x90 at each of 0x35772b 0x35774a 0x35098d
          #    0x350a0a 0x350f5c 0x350f8f 0x3571ee 0x3578cc 0x357d61
          #    0x357d7b 0x357fdc 0x1fa3a4 0x243ce5 0x243d45 0x2d8891
          #    0x2da440 0x34a15f).
          #  - SCRIPTS/DeadlockFix.asi: fixes the AB-BA deadlock on
          #    CS@00864F00 during track loading. ./deadlock-fix.c stays
          #    as its source (built with i686-w64-mingw32-gcc -shared
          #    -nostdlib -lkernel32
          ...
          # at SCRIPTS/*; the pinned tree stays the base lower).
