...
Encased's bundled Encased_Data/Plugins/libfmodstudio.so ships with an
executable-stack request that modern Linux kernels refuse, wedging FMOD
audio init so the engine freezes on a black screen right after creating
the swapchain. patchelf < 0.18 has no --clear-execstack, so we strip the
